...
\begin{align*}
\mtext{Average Cost} &= ((a + ((b - a) \times x1) - (a + ((b-a) \times x0))) / 2 \\
&\Leftrightarrow \mtext{Average  Cost} = (a + ((b - a) \times (x1 - x0))) / 2
\end{align*}
...
When a participant broadcasts a carbon trading offer then the type of the \texttt{Offer} (also another 
...
Both classes extend from Presage2's predefined \texttt{EnvironmentService} class, and use Google's Guice injection systems for initialisation and, in the global service's case, to register with the \texttt{EventBus}.
...
The carbon target service retains all the necessary data required for the above calculations in a private \texttt{ArrayList}. Year targets are also set using a package protected method in the \texttt{AbstractCountry} superclass.
...
The balancing of these constants was not a simple task, since changing one may have an impact on others. The following three facts were used as a starting point for calculating suitable values for some of the constants:
